<li id="prologue-<?php the_ID(); ?>" <?php post_class( get_the_author_meta('ID') ); ?>>
		<?php if ( !is_page() ) : ?>
			<?php 
				echo '<div class="avatar_overlay"></div>';
				printf( 
					'<a href="%1$s" title="%2$s">%3$s</a>',
					get_author_posts_url( p2_get_author_id() ),
					sprintf( __( 'Posts by %s', 'p2' ), esc_attr( p2_get_author_name() ) ),
					p2_get_user_avatar( array( 'user_id' => p2_get_author_id(), 'size' => 64 ) )
				);
			?>
		<?php endif; ?>
			<span class="meta">
				<?php if ( !is_page() ) : ?>
					<span class="date-time"><?php echo p2_date_time_with_microformat() ?></span>
				<?php endif; ?>                
			</span>
			<h4>
			<?php if ( !is_page() ) : ?>
