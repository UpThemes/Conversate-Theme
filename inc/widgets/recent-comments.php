...
?>
		<ul class='p2-recent-comments' avatar="<?php echo esc_attr($avatar_size); ?>">
	...
	function single_comment_html($comment, $avatar_size ) {
		...
		$row  = "<li>";
		if ( !$no_avatar) $row .= "<div title='$author_name' class='avatar' style='height: ${avatar_size}px; width: ${avatar_size}px'>" . $avatar . '</div>';
		...
		$row .= sprintf( '<div class="text">'.__( "%s on <a href='%s' class='tooltip' title='%s'>%s</a>" , 'p2') . '</div><div class="clear"></div></li>', $author_html, $comment_url, attribute_escape($comment_content), $post_title );
		return $row;
	}
}
